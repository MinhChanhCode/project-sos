...
def extract_budget(text: str) -> Optional[int]:
    for pattern, value in [
        (r"100[\.,]?000|100k", 100000),
        (r"200[\.,]?000|200k", 200000),
        (r"50[\.,]?000|50k", 50000),
    ]:
...
def rag_search(query: str, items: list[dict]) -> list[dict]:
    ...
    for item in items:
        ...
        if budget and price > budget:
            continue
        if no_spicy and "cay" in desc:
            continue
        if any(kw in name or kw in desc for kw in lower.split() if len(kw) > 2):
            results.append(item)
        elif "combo" in lower and "combo" in name:
            results.append(item)
        elif "phở" in lower or "pho" in lower:
...
def build_rag_reply(message: str) -> str:
    matched = rag_search(message, MENU_ITEMS)
    if not matched:
    ...
    for m in matched:
        lines.append(f"- {m['name']} ({int(m['price']):,}đ): {m.get('description', '')}")
    return "\n".join(lines)


def analyze_sentiment(text: str) -> SentimentResponse:
    lower = text.lower()
    positive = ["ngon", "tuyệt", "tốt", "hài lòng", "xuất sắc", "great", "good", "excellent"]
    negative = ["tệ", "dở", "chậm", "kém", "bad", "slow", "terrible", "disappoint"]
...
def sync_menu(req: MenuSyncRequest):
    ...
    return {"synced": len(MENU_ITEMS)}


@app.post("/embeddings/rebuild")
def rebuild_embeddings():
    return {"status": "ok", "count": len(MENU_ITEMS), "engine": "faiss-local-rag"}
